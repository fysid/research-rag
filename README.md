...
Локальная RAG-система на базе fork [AnythingLLM](https://github.com/Mintplex-Labs/anything-llm). Вся разработка в этом fork ведётся через LLM-агентов: задача → ветка → проверки → ревью → PR.

**Этап 1:** подготовка репозитория. Локальный запуск и изменения RAG — следующие этапы.

- [Инструкции для LLM](AGENTS.md)
- [Процесс разработки](docs/LLM_DEVELOPMENT.md)
- [План проекта](docs/ROADMAP.md)
- [Обновления upstream](docs/UPSTREAM.md)

Репозиторий публичный; документы, модели, индексы и секреты хранятся локально вне Git. Это самостоятельный fork, не официальный продукт Mintplex Labs. Оригинальная лицензия и документация AnythingLLM сохранены ниже.
